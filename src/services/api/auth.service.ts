...
import { IAuth } from '@hansin91/jobber-shared';
...
class AuthService {
  ...
  async resendEmail(data: { userId: string, email: string }): Promise<AxiosResponse> {
  ...
  async forgotPassword(email: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await this.axiosService.axios.put('/forgot-password', { email, action: 'forgotPassword' });
    return response;
  }

  async resetPassword(token: string, password: string, confirmPassword: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await this.axiosService.axios.put(`/reset-password/${token}`, { password, confirmPassword, action: 'resetPassword' });
    return response;
  }

  async changePassword(currentPassword: string, newPassword: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosAuthInstance.put('/change-password', { currentPassword, newPassword, action: 'changePassword' });
    return response;
  }
